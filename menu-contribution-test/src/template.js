...
console.log(JSON.stringify({
    ...
    contributes: {
        commands: supportedContributionPoints.map(point => ({ command: toCommandId(point), title: `Appears in: ${point}`, icon: '$(beaker)' })).concat({ command: 'menu-contribution-test.thumbsDown', title: 'Make the thumb point down', icon: '$(thumbsup)' }, { command: 'menu-contribution-test.thumbsUp', title: 'Make the thumb point up', icon: '$(thumbsdown)' }),
